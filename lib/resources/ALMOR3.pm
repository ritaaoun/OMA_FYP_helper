...
our @ISA = qw(Exporter);
our @EXPORT_OK = qw( initialize analyzeSolutions generateSolutions isPunct analyzeSolutionsWithConstraints );
...
sub initialize {
  ...
  elsif ($mode!~/^(analysis|generation)$/i){
  ...
  while (my $entry = <DB>){
	  ...
	  my $undiac=$1;
	  my $cat=$2;
	  #if( $saveentry =~ /NOAN_0/ ) { print STDERR "ENTRY [$saveentry][$entry]\n"; }

	  my $len = length($undiac);
	  if( $sublex eq "pre#" ) { 
	      if( $len > $hash{"#MAXPRELEN#"} ) {
		  $hash{"#MAXPRELEN#"} = $len;
	      }
	  } elsif ($sublex eq "suf#" ) {
	      if( $len > $hash{"#MAXSUFLEN#"} ) {
		  $hash{"#MAXSUFLEN#"} = $len;
	      }
	  }
	  ...
	  if ($mode=~/analysis/i){
	      ...
	      my $temp = "$entry XAMACAT:$cat XAMASURF:$undiac";
	      #$temp =~ s/\s+/ /g;
	      #$temp =~ s/^\s+|\s+//g;
	      my %FS= %{ &featureStr2Hash($temp) };
...
=cut
		  ...
		  foreach my $f (keys %FS){
		      my $v=$FS{$f};
		      if ($f eq ""){ #THERE IS A WEIRD BUG HERE: no feature cases are Pref-0 and Suf-0
			  $v="";
			  push @{$hash{"$sublex#FEAT#$f:$v"}},\%FS;
			  #print STDERR "$sublex#FEAT#$f:$v --> [$saveentry][$entry]\n";
			  #foreach my $en (keys %FS) {
			  #    print STDERR "  K:[$en] --> V:[$FS{$en}]\n";
			  #}
		      }elsif ($hash{"Xfeat $f"}){#only count features that are real
			  my $v=$FS{$f};
			  $Xsize++;
  ...
  #print STDERR " MAX PRE LEN = " . $hash{"#MAXPRELEN#"} . "\n";
  #print STDERR " MAX SUF LEN = " . $hash{"#MAXSUFLEN#"} . "\n";
  return(\%hash);
}

##################################################################################

=head3 analyzeSolutionsWithContraints

    ALMOR3::analyzeSolutionsWithContraints($word,$DBhashref,$backoffmode,$contraints)
    ...
    my $word = "Ely";
    my $diac = "EalaYN";
    my @constraints = ( ["diac",$diac], ["pos","noun"] );
    my $backoffmode = "add-all";
    my ($result, $analysisref) = &ALMOR3::analyzeSolutionsWithContraints($word,$DBhashref,$backoffmode,\@contraints);

    ## $analysisref now points to the lexical generated analyses (including spelling variations),
    ##  and every addition analysis that can be generated from affixes (the most exhaustive backoff mode)
    ## However, only analysis which match the given contraints exactly (diac=EalaYN and pos=noun) will
    ##  be returned.  If no-analysis matches the contraints, all the analyses will be returned.
    ...
    Returns a result value, a reference to an array of analyses, and a string.  If $result = 0, then no analysis
    matched all the constraints and all were returned.  Otherwise some subset of the original analysis
    set did match and only those were returned.

    The returned string contains a concatentation of the feat=cval terms, followed by a :0 or :1. The :1 indicates
    that some analyses considered passed the filter; :0 indicates none did.

=cut

sub analyzeSolutionsWithConstraints { # returns a list of 1 or more solutions
    my ($word,$hash, $backoff, $constraintref )=@_;

    ## Get original set of analyses
    my $base = &analyzeSolutions($word,$hash,$backoff);
    if( scalar( @{ $base } ) < 1 ) {
	## No analyses to begin with -- return empty array reference
	return (-1, $base, "");
    }

    my @keep = ();
    my @keep2 = ();
    my ($i,$j, $feat, $cval, $val);
    my ($match, $fails);
    my $reduction = 0;
    my $cresult = "";
    my $usesukun=1;
    my $norm = 0;
    for($i=0;$i<=$#{$base}; $i++) {
	if( $base->[$i] !~ /ta\/PREP/ ) {   ## Discard ta/PREP analyses immediately
	    push @keep, $base->[$i];
	}	
    }
    if( scalar(@keep) == 0 && scalar(@{$base}) > 0 ) {
	## only had ta/PREP analyses, so keep them
	@keep = @{ $base };
    }

    for($j=0; $j<=$#{$constraintref}; $j++) {
	$feat =  $constraintref->[$j]->[0];
	$cval = $constraintref->[$j]->[1];
	
	for($i=0;$i<=$#keep; $i++) {
	    
	    ($match,$fails) = &_constraintMatch($feat,$cval,$keep[$i]);
	    if( ! $match ) {
		$keep[$i] .= " $fails";
	    } else {
		push @keep2, $keep[$i];
	...
	if( scalar(@keep2) > 0 ) {
	    ## One or more analyses matched the constraint, so only examine
	    ##  those from this point forward
	    @keep= @keep2;
	    $reduction = 1;  ## there was at least some filtering from the original
	    $cresult .= " $feat=$cval:1";
	} else {
	    $cresult .= " $feat=$cval:0";
	}
    ...
    return ($reduction, \@keep, $cresult);

}

##################################################################################

=head3 _constraintMatch
    
    ALMOR3::_constraintMatch($feat, $cval, $analysis)

    my ($match, $fails) = _constraintMatch($feat, $cval, $analysis);

    if( ! $match ) {
      print  "Failed on conditions $fails\n";
      ...
    }

    Returns 1 if the given the $feat of the given $analysis line matches 
    (either exactly or softly) the specified $cval; returns 0 otherwise.

    The returned $fails string shows the final compared values if $match == 0;
    it is empty otherwise.

    $feat can be any standard analysis feature, or a modified lex/diac (with 
    "nosukun-" and/or "norm").  For $feat="pos",  a $cval of NOM, VERB or
    NOUN_PROP can also be used.

=cut

sub _constraintMatch {
    my ($feat, $cval, $analysis) = @_;
    my $match = 1;
    my $fails = "";
    my $usesukun=1;
    my $norm = 0;

    ## Normalize/soften constriant value as needed
    if( $feat =~ /norm/ && $feat =~ /(lex|diac)/ ) {
	$norm = 1;
	$cval =~ s/Y/y/g; 
	$cval =~ s/[><\|A\{]/A/g;
	$feat =~ s/norm//;
	...
	$cval =~ s/iy/y/g;
	$cval =~ s/uw/w/g;
	$cval =~ s/((A|l|Al)l\S)\~/$1/g;  ## Sun letters
    }

    if( $feat =~ /nosukun\-/ && $feat =~ /(lex|diac)/ ) {
	$usesukun = 0;
	$cval =~ s/o//g;
	$feat =~ s/nosukun\-//;
    }
    ...
    ## Get the analysis value & apply same normalization/softening & compare
    my $val = "";
    if( $feat eq "pos"  ) {
	if( $analysis =~ /(^|\s)$feat\:(\S+)/ ) {
	    $val = $2;
	    #print STDERR "WORD $word  $feat  Cval=$cval  val=$val \n";

	    if( $cval eq "verb" ) {
		if( $val !~ /^(verb|verb_pseudo)$/i ) {
		    $match = 0;
		    $fails = "fail:match-verb:$cval-$val";
		}	
	    } elsif( $cval eq "nom" ) {
		if( $val !~ /^(adj|adj_comp|adj_num|noun|noun_num|noun_quant|adv|adv_interrog|adv_rel)$/i ) {
		    $match = 0;
		    $fails = "fail:match-nom:$cval-$val";
		}	
	    } elsif( $cval eq "noun_prop" ) {
		if( $val !~ /^(noun_prop)$/i ) {
		    $match = 0;
		    $fails = "fail:match-noun_prop:$cval-$val";
		}	
	    } elsif( $cval eq "not-noun_prop" ) {
		if( $val =~ /^(noun_prop)$/i ) {
		    $match = 0;
		    $fails = "fail:match-not-noun_prop:$cval-$val";
		}	
	    }
	}	         
    }elsif( $analysis =~ /(^|\s)$feat\:(\S+)/ ) {
	$val = $2;
	if( $norm && $feat =~ /(diac|lex)/ ) {
	    $val =~ s/Y/y/g; 
	    $val =~ s/[><\|A\{]/A/g;
	}
	if( $feat =~ /(lex|diac)/ ) {
	    ...
	    $val =~ s/iy/y/g;
	    $val =~ s/uw/w/g;
	    $val =~ s/((A|l|Al)l\S)\~/$1/g;  ## Sun letters
	}	    
	if( $usesukun == 0 && $feat =~ /(diac|lex)/ ) {
	    $val =~ s/o//g;
	}
	...
	if( $val ne $cval ) {
	    $match = 0;
	    $fails = "fail:match-$feat:$cval-$val";
	} 
    }
...
sub analyzeSolutions { # returns a list of 1 or more solutions
    ...
    }elsif (( $word=~/[BCceGIJLMOPQRUVWX]/ ) || ($word=~/^(\s|[aiuo\~\`FKN])*$/)){
	##  Word is likely a non-Arabic word
	my %default=%{$$hash{"DEFAULT pos:noun"}};
	$default{"diac"}=$word; $default{"lex"}=$word."_0"; 
	$default{"bw"}=$word."/FOREIGN"; $default{"gloss"}=$word; $default{"source"}="foreign";
	my $analysis=&featureHash2Str($$hash{"ORDER"},\%default);
	return [$analysis];
	
    }elsif ( $word=~/\d+/ ){
	##  Word contains digits
	my %default=%{$$hash{"DEFAULT pos:digit"}};
	$default{"diac"}=$word; $default{"lex"}=$word."_0"; $default{"bw"}=$word."/NOUN_NUM"; 
	$default{"gloss"}=$word; $default{"source"}="digit";
	my $analysis=&featureHash2Str($$hash{"ORDER"},\%default);
	return [$analysis];
	...
	$default{"diac"}=$word; $default{"lex"}=$word."_0"; $default{"bw"}=$word."/PUNC"; 
	$default{"gloss"}=$word; $default{"source"}="punc";
	my $analysis=&featureHash2Str($$hash{"ORDER"},\%default);
	return [$analysis];
    }
	...
	if (exists($$hash{"stem#$stem"})){   ## SHOULD MOVE THIS CHECK INTO SEGMENTWORD? BACKOFF ANALYZE ISSUE?
...
sub generateSolutions { # returns a list of 1 or more solutions
    ...
    if (exists $$hash{"LEX#$lex"}){  ## LEX# only shows up for stems in the almor database
	@base=@{$$hash{"LEX#$lex"}};  ## @base is now array of references to stem feature 
	                              ## hashes from almor database having this lex
    }else{
	...
	if ($lex=~/(\S+)\_0/){
	    ...
	    if (($STEM=~/[BCceGIJLMOPQRUVWX]/)&&($LF{"pos"} eq "noun")){  ## Non-Arabic/FOREIGN
		if ($gmode eq "diac"){ 
		    @output=($STEM);
		}else{
		...
		return \@output;
	    }elsif (($STEM=~/\d+/)&&($LF{"pos"} eq "noun_num")){   ## Digits / NOUN_NUMs
		if ($gmode eq "diac"){ 
		    @output=($STEM);
		}else{
		...
		return \@output;
	    }elsif ((&isPunct($STEM))&&($LF{"pos"} eq "punc")){   ## Punctuation / PUNC
		if ($gmode eq "diac"){ 
		    @output=($STEM);
		}else{
	...
	@base=@{$$hash{"LEX#NOAN_0"}};   ## Not a simple case; No analysis default
	$noan=1;
	...
	## $x is a reference to a feature hash from a stem entry in the almor database
...
#	print "LF>>\t$Z\n";
	...
	#ignore POS mismatch cases... Only consider almor database entries that:
	#    match the supplied pos and
	#    (the input doesn't have a stem) or (it has a stem and the stem  matches)
	if (($$x{"pos"} eq $LF{"pos"})
	    && (((exists $LF{"stem"})&&($$x{"diac"} eq $LF{"stem"}))
		||(not exists $LF{"stem"}))  ){
	    # step 1
	    my $cat_b=$$x{"XAMACAT"};  # Stem Category from the almor database (b ==> stem)
		...
		push @prefixes, $ff;
		#print STDERR "PREF added [" . &featureHash2Str("anyorder", $ff) . "]\n";
	    }
	    #foreach my $pref (@prefixes) {
		#print STDERR "PREF added [" . &featureHash2Str("anyorder", $pref) . "]\n";
		#print STDERR "  Xfeat array has # elements = ". scalar(@{$pref->{Xfeat}}) ."\n";
		#foreach my $f ( @{ $pref->{Xfeat} } ) {
		#    print STDERR "   K:[$f] \n";
		#}
	    #}
		...
		push @suffixes, $ff;
		#print STDERR "SUFF added [" .  &featureHash2Str("anyorder", $ff) . "]\n";
	    }
	    ...
	    foreach my $precat (@{$$hash{"PRECAT $cat_b"}}){   ## $precat is from TABLE AB
		foreach my $prefix (@{$$hash{"FEAT#$precat"}}){  
                    ## $prefix is prefix entry from almor database matching the category $precat
		    my $match=1;
			...
			push @prefixes, $prefix;   
                        ## Record the $prefixes that fully match the input list prefix features 
		    }
		}	      
	    }
	    ...
	    foreach my $sufcat (@{$$hash{"SUFCAT $cat_b"}}){    ## $sufcat if rom TABLE BC
		foreach my $suffix (@{$$hash{"FEAT#$sufcat"}}){ 
		    ## $suffix is a suffix entry from database matching the category $sufcat
		    $$suffix{"Xsize"}=0;
			...
			## If even 1 input suffix feature matches, record the suffix; record the
			## number of suffix features that match in the suffix entry as "Xsize"
		    }
		}	      
	    }
	   ...
	   # my $xxx=@suffixes; print "CNT SUF $xxx\n";
	    ...
	    ## Now have a list of prefixes and suffixes that are compatible with stem and match
	    ## input features (at least partially for suffixes; totally for prefixes)
	    foreach my $pre (@prefixes){
		foreach my $suf (@suffixes){
		    #print $$pre{"bw"}."  +  ".$$x{"bw"}."  +  ".$$suf{"bw"} ."\n";
		    my $cat_a=$$pre{"XAMACAT"};
		    my $cat_c=$$suf{"XAMACAT"};
		    if ( exists($$hash{"tAC#$cat_a $cat_c"}) ){    # If there is a $prefix $suffix entry in TABLEAC
			#my $match=$$pre{"Xsize"}+$$x{"Xsize"}+$$suf{"Xsize"};
			my $match=&featureMergePSSandMatch($pre,$x,$suf,\%LF);
			#### $match is the number of input features that match the prefix, suffix, stem
			## feature entries; matches attempted in that order


			#print "$match $maxmatch\n";

			if ($match >= $maxmatch){  
                            ## If we get a match score >= to the biggest one we've found so far			    
			    my $out = $$pre{"diac"}."+".$$x{"diac"}."+".$$suf{"diac"};
			    my $surfdiac=&BWMorphotactics($out);  # $surfdiac is a proposed diac form
			    
			    
			    #DIFFERENT GMODES:
			    ## "diac" -- just output surfdiac form
			    ##  otherwise output surfdiac, the above $out form, and the combined BW tags
			    if ($gmode eq "diac"){
				$out= "$surfdiac";
			    }else{
			    ...
			    if ($match > $maxmatch){     ## only keep the top-scorers
				%output=();
				$output{$out}=1;
				$maxmatch=$match;
			    }elsif ($match==$maxmatch){  ## might be more than one (tie top score)
				$output{$out}=1;
			    }
			}
		    }
		} #end: foreach my $suf (@suffixes){
	    } #end: foreach my $pre (@prefixes){

	} #end: if (($$x{"pos"} eq $LF{"pos"})...
    } #end: foreach my $x (@base){ ...
...
# based on a function from BAMA
    ...
    #my $max_prefix_len = 4;
    #my $max_suffix_len = 6;
    my $max_prefix_len = $hash->{"#MAXPRELEN#"};
    my $max_suffix_len = $hash->{"#MAXSUFLEN#"};
	...
	if (exists($$hash{"pre#$prefix"})) {
	    ...
	    $suffix_len = 0;
	    while (($stem_len >= 1) and ($suffix_len <= $max_suffix_len)) {
...
sub featureStr2Hash {
    my ($x)=@_;
    my %x=(); 
    #if( $x =~ /\:(Suff|Pref)-0/ ) { print STDERR "FS2H  =  [$x]\n"; }
